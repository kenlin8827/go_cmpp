...
var (
	...
	activeTest          = flag.Bool("activeTest", true, "是否activeTest")
...
func (s *Session) mockReport(submit *protocol.Submit) error {
	candidateStats := []string{"DELIVRD", "UNDELIV"}
	for _, destTerminalId := range submit.DestTerminalId {
		// report
		r := rand.New(rand.NewSource(time.Now().Unix()))
		idx := r.Intn(len(candidateStats))
		randomStat := candidateStats[idx]
		...
		case protocol.CMPP_SUBMIT:
			...
			if submit.RegisteredDelivery == 1 {
				go func() {
					...
					deliverRptBroadcast.Submit(&DeliverRequest{
...
func (s *Session) deliverWorker() {
	...
	for msg := range deliverRptChan {
		deliverRequest := msg.(*DeliverRequest)
		submit := deliverRequest.submit
		if deliverRequest.sourceAddr != s.sourceAddr {
			continue
		}
		if *rpt {
